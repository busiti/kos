...
function statusText(nextDue, today){
  ...
  if (months === 0 && days === 0) return `<span class="bg-blue-500/10 text-blue-400 px-2 py-1 rounded-md text-[10px] font-bold uppercase">Belum jatuh tempo</span>`;
...
async function build(){
  ...
  for (const u of units){
    ...
    if (!t){
      rows.push(`
        <tr class="hover:bg-white/5 transition">
          ...
          <td class="p-4 border-b border-white/5 text-[#8aa0c6]">—</td>
          <td class="p-4 border-b border-white/5"><span class="text-[10px] text-white/30 uppercase font-bold tracking-widest">Kosong</span></td>
    ...
    const btns = [];
    const btnClass = "px-3 py-1 bg-white/5 border border-white/10 rounded-md text-[10px] font-bold hover:bg-emerald-500/20 hover:text-emerald-400 transition";
  ...
  const renderBucket = (arr, elId, tipe) => {
    ...
    if (!arr.length){ el.innerHTML = `<div class="text-[#8aa0c6] text-xs italic">Tidak ada antrean.</div>`; return; }
    el.innerHTML = arr.map(x=>{
      ...
      return `
        <div class="flex justify-between items-center py-2 border-b border-white/5 last:border-0 group">
          ...
          <a class="px-3 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-lg text-[10px] font-bold hover:bg-emerald-500 hover:text-white transition" 
             href="${waLink(x.phone, text)}" target="_blank">WA</a>
        </div>
      `;
    }).join("");
  };
